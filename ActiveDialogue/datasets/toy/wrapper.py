...
def load_dataset(splits=('train', 'dev', 'test')):
    with open(os.path.join(mnt_dir + "/toy/ann", 'ontology.json')) as f:
        ontology = Ontology.from_dict(json.load(f))

    with open(os.path.join(mnt_dir + "/toy/ann", 'vocab.json')) as f:
        vocab = Vocab.from_dict(json.load(f))

    with open(os.path.join(mnt_dir + "/toy/ann", 'emb.json')) as f:
        E = json.load(f)
